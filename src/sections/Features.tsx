...
const FeatureTab = (
  ...
  const iconRef = useRef<DotLottieCommonPlayer>(null);
  ...
  useEffect(() => {
    ...
    const parameter = 2 * (height + width);

    const times = [
      0,
      width / parameter,
      (width + height) / parameter,
      (2 * width + height) / parameter,
      1,
    ];
...
export const Features = () => {
  ...
  const handleSelectedTab = (idx: number) => {
    setselectedTab(idx);
    animate(
      backgroundSizeX,
      [backgroundSizeX.get(), 100, tabs[idx].backgroundSizeX],
      { duration: 2, ease: "easeInOut" }
    );
    animate(
      backgroundPositionX,
      [backgroundPositionX.get(), tabs[idx].backgroundPositionX],
      { duration: 2, ease: "easeInOut" }
    );
    animate(
      backgroundPositionY,
      [backgroundPositionY.get(), tabs[idx].backgroundPositionY],
      { duration: 2, ease: "easeInOut" }
    );
  };
